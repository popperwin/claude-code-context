...
# Legacy scan_mode import removed - functionality replaced by delta-scan
...
class TestHybridIndexerIntegration:
    ...
    async def test_error_handling_in_indexing(self, tmp_path):
        ...
        # Create mock that raises exception during delta-scan
        mock_parser = Mock()
        mock_parser.registry = Mock()
        mock_parser.max_workers = 4
        mock_parser.batch_size = 10
        ...
        # Mock the delta-scan method to raise an exception
        indexer.perform_delta_scan = AsyncMock(side_effect=Exception("Delta-scan failed"))
        
        config = IndexingJobConfig(
            project_path=tmp_path,
            project_name="error-test",
            enable_delta_scan=True  # Explicitly enable delta-scan mode
        )
        ...
        assert "Delta-scan failed" in metrics.errors[0]
